...
// === GALLERY GRID REDESIGN: 1 row + fast crossfade ===
add_action('wp_footer', function() {
    if (!is_page('models') && !is_page('hostess-steward') && !is_page('actors')) return;
    echo '<style>
.gallery-grid.cols-4 {
...
.gallery-item img.gi-a,
.gallery-item img.gi-b {
...
(function(){
  ...
  // Collect ALL image sources from all items (including hidden ones)
  var allSrcs = [];
  allItems.forEach(function(item){
    var img = item.querySelector("img");
    if(img && img.src) allSrcs.push(img.src);
  });
  if(allSrcs.length < 2) return;

  // Preload all
  allSrcs.forEach(function(s){ var p = new Image(); p.src = s; });

  // Only work with visible items (first 8)
  var items = allItems.slice(0, 8);

  // Setup dual layers
  var cells = [];
  items.forEach(function(item, i){
    var origImg = item.querySelector("img");
    var srcA = origImg ? origImg.src : allSrcs[i % allSrcs.length];
    origImg.className = "gi-a";
    origImg.style.setProperty("opacity","1","important");

    var imgB = origImg.cloneNode(false);
    imgB.className = "gi-b";
    imgB.style.setProperty("opacity","0","important");
    item.appendChild(imgB);

    cells.push({ a: origImg, b: imgB, showing: "a" });
  });

  var currentMap = cells.map(function(c){ return c.a.src; });

  function crossfadeCell(idx){
    var cell = cells[idx];
    var usedSrcs = currentMap.slice();
    var available = allSrcs.filter(function(s){ return usedSrcs.indexOf(s) === -1; });
    if(available.length === 0){
      available = allSrcs.filter(function(s){ return s !== currentMap[idx]; });
    }
    if(available.length === 0) return;
    var newSrc = available[Math.floor(Math.random() * available.length)];

    if(cell.showing === "a"){
      cell.b.onload = function(){
        cell.a.style.setProperty("opacity","0","important");
        cell.b.style.setProperty("opacity","1","important");
        cell.showing = "b";
        currentMap[idx] = newSrc;
      };
      cell.b.src = newSrc;
    } else {
      cell.a.onload = function(){
        cell.b.style.setProperty("opacity","0","important");
        cell.a.style.setProperty("opacity","1","important");
        cell.showing = "a";
        currentMap[idx] = newSrc;
      };
      cell.a.src = newSrc;
  ...
  // Fast cycle: swap one cell every ~800ms-1200ms
  var cellIdx = 0;
  function tick(){
    var order = [];
    for(var i=0;i<cells.length;i++) order.push(i);
    // Shuffle
    ...
    function nextSwap(){
      ...
      crossfadeCell(order[pos]);
      pos++;
      setTimeout(nextSwap, 800 + Math.random() * 600);
    }
    nextSwap();
  }
